...
#| include: false
# load required packages for scRNA-seq analysis
library(future)
...
```{r include = F}
# configure Python environment for S3 access via boto3
# NOTE: users must set ATTEMPT_PYTHON_PATH to their Python installation
reticulate::use_python(Sys.getenv("ATTEMPT_PYTHON_PATH", unset = "python3"), required = T)
reticulate::repl_python()
reticulate::py_config()

# set up Kopah S3 connection via boto3
...
# create S3 client for Kopah storage
...
# map KPMP subtypes to general cell type categories
...
```
```{r echo = F}
# map general cell types to module score column prefixes
...
# assign each cell its cell-type-specific module score
...
```{r echo = F}
# fit mixed models for module scores with treatment x visit DiD interaction
attempt_so_meta <- attempt_so@meta.data

# fit immune module score mixed model and get DiD contrasts by cell type
...
# fit metabolic module score mixed model and get DiD contrasts by cell type
...
```{r}
# forest plot of immune score DiD estimates by cell type
immune_contrasts_df %>%
...
# forest plot of metabolic score DiD estimates by cell type
...
```{r echo = F}
# set factor levels for treatment and visit
...
```{r echo = F}
# create UMAP plots colored by visit, treatment, and cell type
...
```{r echo = F}
# collapse cell subtypes into broad categories for proportion analysis
...
# extract metadata and set factor levels for plotting
...
```{r echo = F}
# bar plots of cell counts and proportions by visit x treatment
attempt_meta %>%
...
# compute cell counts per general cell type for annotation labels
...
# proportion plot for TAL KPMP subtypes
...
```{r echo = F}
# select top 2000 highly variable genes across all cells
...
```{r echo = F}
# summarize subject-level demographics by visit x treatment
...
# PT
```{r echo = F}
# subset to proximal tubule cells
...
```{r echo = F}
# find HVGs and subset PT cells to top 2000
...
#### Proportions
```{r echo = F, eval = F}
# stacked bar plots of PT subtype proportions by visit and treatment
...
```{r echo = F, eval = F}
# Dirichlet regression on PT subtype proportions by treatment arm
...
```{r echo = F, eval = F}
# Dirichlet regression on PT proportions using KPMP cell type labels
pt_props <- attempt_meta %>%
  filter(KPMP_celltype %in% c("PT-S1/S2", "PT-S3", "aPT")) %>%
...
```{r echo = F}
# custom barplot and density CI functions for scDC proportion bootstrap results
...
```{r echo = F}
# test PT subtype proportion shifts using propeller (logit-transformed proportions + limma)
library(speckle)
...
```{r echo = F}
# propeller proportion test using KPMP PT subtypes
# Placebo
attempt_so_pt_placebo <- subset(attempt_so_pt, treatment == "Placebo")
...
```{r echo = F}
# run MWU proportion tests for PT subtypes in each treatment arm
...
```{r echo = F}
# run MWU proportion tests using KPMP PT subtypes
...
```{r echo = F}
# prepare count matrices for scCODA compositional analysis in Python
...
```{r echo = F, eval = F}
# run NEBULA NBLMM on HVG PT cells for treatment x visit interaction
...
```{r echo = F, eval = F}
# run NEBULA per PT subtype in parallel (HVG subset)
...
```{r echo = F, eval = T}
# run NEBULA on all PT genes (not reduced to HVG)
nebula_pt_results_list_reml_pooled <- run_nebula_parallel(
...
##### KPMP celltype
```{r echo = F}
# subset to PT cells using KPMP labels (exclude EC-PTC mismap)
...
```{r echo = F, eval = F}
# run NEBULA on all KPMP PT genes (not reduced to HVG)
...
```{r echo = F}
# subset to thick ascending limb cells
...
```{r echo = F, eval = F}
# run NEBULA NBLMM per gene on TAL HVGs with treatment x visit interaction
...
```{r echo = F, eval = T}
# run NEBULA NBLMM per gene on all TAL genes (not reduced to HVG)
...
```{r echo = F}
# subset to TAL cells using KPMP labels
...
```{r echo = F, eval = F}
# run NEBULA on all KPMP TAL genes
...
```{r echo = F, eval = F}
# run NEBULA NBLMM per gene on immune HVGs
...
```{r echo = F, eval = T}
# run NEBULA on all immune genes (not reduced to HVG)
...
```{r echo = F}
# subset to immune cells using KPMP labels
...
```{r echo = F, eval = F}
# run NEBULA on KPMP immune HVGs
...
```{r echo = F, eval = F}
# run NEBULA on all KPMP immune genes
...
```{r echo = F, eval = F}
# run NEBULA NBLMM per gene on myeloid immune HVGs
...
```{r echo = F, eval = T}
# run NEBULA on all myeloid immune genes
...
```{r echo = F, eval = F}
# run NEBULA NBLMM per gene on lymphoid immune HVGs
...
```{r echo = F, eval = T}
# run NEBULA on all lymphoid immune genes
...
```{r echo = F, eval = F}
# run NEBULA NBLMM per gene on PC HVGs
...
```{r echo = F, eval = F}
# run NEBULA NBLMM per gene on IC HVGs
...
```{r echo = F, eval = F}
# run NEBULA NBLMM per gene on EC HVGs
...
```{r echo = F}
# subset to fibroblast and vascular smooth muscle cells
...
```{r echo = F, eval = F}
# run NEBULA NBLMM per gene on FIB/VSMC HVGs
...
```{r echo = F, eval = T}
# run NEBULA on all FIB/VSMC genes
...
```{r echo = F, eval = T}
# run NEBULA NBLMM per gene on POD HVGs
...
```{r echo = F}
# re-source functions (in case of updated definitions)
source(here("attempt_functions.R"))
# define cell type groups for grouped analysis
...
```{r echo = F}
# run NEBULA on broad epithelial and endo/glom combined groups (HVG + all genes)
celltype_groups <- list(
...
```{r echo = F}
# define cell type groups matching urine and plasma proteomics compartments
...
```{r echo = F}
# run NEBULA on proteomics-specific cell type groupings
nebula_kpmp_proteomics_specific <- list()

# --- Helper function to run ALL analysis
run_for_subset <- function(so_subset, label) {
  # --- ALL run
  res_all <- run_nebula_parallel(
    seurat_obj = so_subset,
...
```{r echo = F}
# download nonconverged gene lists from NEBULA for each cell type and model
...
### PT
```{r echo = F}
# quantify expression of nonconverged genes in PT cells and plot by model
...
### TAL
```{r echo = F}
# quantify expression of nonconverged genes in TAL cells and plot by model
...
### immune
```{r echo = F}
# quantify expression of nonconverged genes in immune cells and plot by model
...
### PC
```{r echo = F}
# quantify expression of nonconverged genes in PC cells and plot by model
...
### IC
```{r echo = F}
# quantify expression of nonconverged genes in IC cells and plot by model
...
### EC
```{r echo = F}
# quantify expression of nonconverged genes in EC cells and plot by model
...
### FIB, VSMC/P
```{r echo = F}
# quantify expression of nonconverged genes in FIB/VSMC cells and plot by model
...
# HVGs (union)
```{r echo = F}
# combine HVGs from all cell types into a single union set
hvgs_union <- unique(c(hvgs_ec, hvgs_fibvsmc, hvgs_ic, hvgs_immune, hvgs_pc, hvgs_pt, hvgs_tal))
```

# Dot plot of markers
```{r echo = F}
# check HAVCR1 (KIM-1) differential expression in placebo vs dapa
...
# dot plots of kidney injury markers across PT subtypes by treatment x visit
