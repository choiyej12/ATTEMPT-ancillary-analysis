...
#| include: false
# load required packages for pseudotime and trajectory analysis
library(future)
...
```{r echo = F, eval = T}
# download cleaned Seurat object from S3 for pseudotime analysis
...
```{r echo = F}
# run slingshot pseudotime for PT KPMP subtypes, starting from PT-S1/S2
...
# density plots of pseudotime by HbA1c change direction, weighted per subject
...
# density plots of pseudotime by TIR change direction
...
# density plots of pseudotime by mGFR change direction
...
# mixed-effects models of pseudotime vs clinical variables (random intercept per subject)
...
# per-subject weighted density of pseudotime by treatment group
dat <- dat %>%
  group_by(subject_id, visit) %>%
  mutate(weights = 1/n()) %>%
  ungroup()
...
```{r}
# compute POST minus PRE weighted kernel density difference along pseudotime
...
# plot density difference curves (POST - PRE) for each clinical variable direction group
...
# create density difference plots for each clinical variable
...
# density difference by treatment group instead of clinical variable direction
...
```{r echo = F}
# compositional pie charts showing cell type proportions along pseudotime bins
...
# compositional pie charts of treatment x visit groups along pseudotime bins
...
# loop over clinical variables to create combined pseudotime + pie chart panels
all_outputs <- list()
...
```{r echo = F}
# slingshot pseudotime for PT non-KPMP subtypes (PT-1 through PT-5)
...
```{r echo = F}
# slingshot pseudotime for TAL KPMP subtypes
...
```{r echo = F}
# slingshot pseudotime for TAL + ATL KPMP subtypes
...
```{r echo = F}
# slingshot pseudotime for TAL non-KPMP subtypes
...
```{r echo = F}
# slingshot pseudotime for TAL + ATL non-KPMP subtypes
...
```{r echo = F}
# slingshot pseudotime for IC KPMP subtypes
...
```{r echo = F}
# slingshot pseudotime for IC non-KPMP subtypes
...
```{r echo = F}
# slingshot pseudotime for EC KPMP subtypes
...
```{r echo = F}
# slingshot pseudotime for EC non-KPMP subtypes
