...
# cell type groupings mapping KPMP subtypes to broader cell type groups
...
# read CROCODILE HVG (top 2000) nebula results from S3 for each cell type
# these are T1D vs HC comparisons from the CROCODILE cohort
for (cell in names(celltype_groups))  {
  ...
  write.csv(processed$results, file.path(results_dir, "nebula", paste0(tolower(cell), "_kpmp_croc_hvg_res.csv")), row.names = F)
}

# read CROCODILE targeted gene set nebula results (genes significant in ATTEMPT)
for (cell in names(celltype_groups))  {
  ...
  write.csv(processed$results, file.path(results_dir, "nebula", paste0(tolower(cell), "_kpmp_croc_targeted_res.csv")), row.names = F)
}
```

```{r echo = F}
## read mixed model results dataframe (rendered in Hyak)
# read ATTEMPT DiD nebula results for comparison with CROCODILE
celltype_groups <- list(
  PT = c("PT-S1/S2", "PT-S3", "aPT"),
  TAL = c("C-TAL-1", "C-TAL-2", "aTAL", "dTAL"),
  PC = c("CCD-PC", "CNT-PC", "dCCD-PC", "M-PC", "tPC-IC"),
...
```{r echo = F}
# volcano plots for targeted genes: CROCODILE T1D vs HC with ATTEMPT DiD direction labels
for (cell in names(celltype_groups)) {
  plot_croc_attempt_volcano(
    ...
    cell_type = cell,
    top_n = 20,
    save_path = file.path(results_dir, "Figures/CROCODILE comparison/Volcano Plots", paste0("kpmp_", tolower(cell), "_targeted_volcano_attempt_directions_labeled.png"))
  )
}
```

```{r echo = F}
# standalone CROCODILE targeted volcano plots (raw p-value and FDR versions)
for (cell in names(celltype_groups)) {
  plot_volcano(
...
```{r echo = F}
# volcano plots for untargeted HVG set: CROCODILE with ATTEMPT direction labels
for (cell in names(celltype_groups)) {
  plot_croc_attempt_volcano(
    attempt_df = get(paste0(tolower(cell), "_kpmp")),
    ...
    cell_type = cell,
    top_n = 20,
    save_path = file.path(results_dir, "Figures/CROCODILE comparison/Volcano Plots", paste0("kpmp_", tolower(cell), "_untargeted_volcano_attempt_directions_labeled.png"))
  )
}
```

```{r echo = F}
# standalone CROCODILE HVG volcano plots (raw p-value and FDR versions)
for (cell in names(celltype_groups)) {
  plot_volcano(
...
# manual functional gene groupings for heatmap bracket annotations per cell type
...
# assign each gene to its functional group, defaulting to "Other" if not listed
...
# compute y-positions for functional group bracket annotations on the heatmap
...
# create side-by-side heatmaps of top 50 reversed genes (CROCODILE vs ATTEMPT logFC)
# genes ordered by functional group, with bracket annotations on right side
for (cell in names(celltype_groups)) {
  top_n = 50
  target_n = top_n*2
  # merge ATTEMPT and CROCODILE results, classify genes as reversed or non-reversed
  ...
  # select top 50 reversed genes by CROCODILE p-value
  ...
  # pivot to long format with one row per gene per study for heatmap plotting
  ...
  # add invisible placeholder rows so all cell type heatmaps have uniform height
...
# combine all cell type heatmaps into one panel figure
...
print(final_plot)
ggsave(file.path(results_dir, "Figures/Panels/croc_attempt_rev_heatmap_2.png"), width = 12, height = 12)

```

```{r echo = F}
# alternative heatmaps: same reversed genes but ordered by logFC difference instead of functional groups
for (cell in names(celltype_groups)) {
  top_n = 50
  target_n = top_n*2
...
ggsave(file.path(results_dir, "Figures/Panels/croc_attempt_rev_heatmap.png"), width = 12, height = 12)
```