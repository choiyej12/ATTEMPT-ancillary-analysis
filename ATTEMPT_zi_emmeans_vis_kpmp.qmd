...
# contaminating genes to filter out per cell type (likely from ambient RNA)
...
## read mixed model results dataframe (rendered in Hyak)
# connect to Ensembl for gene annotations
mart <- useMart("ensembl", dataset = "hsapiens_gene_ensembl")

# read grouped HVG nebula results, filter contaminants, annotate with Ensembl gene descriptions
for (cell in names(celltype_groups))  {
  ...
  write.csv(annotated_df, file.path(results_dir, "nebula", paste0(tolower(cell), "_kpmp_hvg_res.csv")), row.names = F)
}

# read individual subtype HVG nebula results, filter contaminants, annotate
for (cell in unlist(celltype_groups))  {
  ...
  write.csv(annotated_df, file.path(results_dir, "nebula", paste0(tolower(cell_clean), "_kpmp_hvg_res.csv")), row.names = F)
}
```

```{r echo = F, eval = F}
# save nebula results as Excel workbooks (top 50 and all genes per cell type)
...
saveWorkbook(all_wb, file = file.path(results_dir, "all_hvg_nebula_results_by_kpmp_celltype.xlsx"), overwrite = TRUE)
saveWorkbook(top50_wb, file = file.path(results_dir, "top50_hvg_nebula_results_by_kpmp_celltype.xlsx"), overwrite = TRUE)
```

#### IPA input save

```{r echo = F, eval = F}
# save formatted CSVs for Ingenuity Pathway Analysis (IPA) input
for (cell in names(celltype_groups)){
  ## HVG
  ...
  write.csv(hvg_ipa_DiD, file.path(results_dir, "emmeans/kpmp/nebula",
                            paste0(tolower(cell), "_nebula_pooled_ipa_input_DiD.csv")), row.names = F)
  ...
  write.csv(ipa_DiD, file.path(results_dir, "emmeans/kpmp/nebula",
                            paste0(tolower(cell), "_nebula_pooled_ipa_input_DiD_full.csv")), row.names = F)
...
```{r echo = F}
# volcano plots for DiD interaction term (treatment x visit) per cell type
# HVG gene set
for (cell in names(celltype_groups)) {
...
```{r echo = F}
# combined volcano + subtype dot plot for PT cell types
pt_plots <- create_gene_expression_plots(
  ...
  formula = "treatment * visit",
  output_dir = file.path(results_dir, "Figures/Dot Plots")
)
...
```{r echo = F}
# combined volcano + subtype dot plot for TAL cell types
...
```{r echo = F}
# combined volcano + subtype dot plot for PC cell types
...
```{r echo = F}
# combined volcano + subtype dot plot for all immune cell types
...
```{r echo = F}
# combined volcano + subtype dot plot for myeloid immune subtypes
...
```{r echo = F}
# combined volcano + subtype dot plot for lymphoid immune subtypes
...
```{r echo = F}
# combined volcano + subtype dot plot for IC cell types
...
```{r echo = F}
# combined volcano + subtype dot plot for EC cell types
...
```{r echo = F}
# combined volcano + subtype dot plot for VSMC/P and FIB
...
```{r echo = F}
# volcano + dot plot for POD (HVG gene set)
...
```{r echo = F}
# volcano + dot plot for POD (full gene set)
...
```{r echo = F}
# combine all cell types into one df for cross-cell-type jitter plot
...
# jitter plot of DiD logFC across cell types, colored by FDR significance
combined_nebula_reml_pooled %>%
  filter(`logFC_treatmentDapagliflozin:visitPOST` < 10) %>%
...
ggsave(file.path(results_dir, "Figures/Cell type comparison", "celltype_deg_combined_kpmp.png"), width = 5, height = 4, dpi = 600)


# find genes significant (FDR < 0.05) in more than one cell type
...
# jitter plot with TCA cycle gene labels highlighted
tca_genes <- c(
...
# jitter plot with oxidative phosphorylation gene labels highlighted
oxphos_genes <- c(
...
# combine all cell types and count positive/negative DEGs (FDR < 0.05)
...
save(deg_summary_wide, file = file.path(results_dir, "model comparison", "celltype_deg_comparison_kpmp.RData"))

# stacked barplot of positive and negative DEG counts per cell type
deg_summary %>%
...
ggsave(file.path(results_dir, "Figures/Cell type comparison", "celltype_deg_stacked_barplot_kpmp.png"),
...
```{r echo = F}
# upset plot showing overlap of DEGs (FDR < 0.05, |logFC| > 0.5) across cell types
...
ggsave(filename = file.path(results_dir, "Figures/Cell type comparison", "celltype_deg_upset_kpmp.png"),
       width = 5, height = 10, dpi = 300,  bg = "transparent")

```

```{r echo = F}
# chord diagram of shared DEGs between cell types with blended colors
combined_sig <- combined %>%
  filter(fdr < 0.05) %>%
  dplyr::select(Gene, celltype) %>%
  filter(celltype %nin% c("IMMUNE_MYELOID", "IMMUNE_LYMPHOID"))

# count genes unique to each cell type for self-links in the chord diagram
...
# build pairwise shared gene count matrix for chord diagram
...
# blend two cell type colors for chord diagram ribbon colors
...
# Open PNG device
png(file.path(results_dir, "Figures/Proportions", "celltype_deg_chordplot.png"), width = 800, height = 800, res = 150)
...
```{r echo = F}
# cross-cell-type concordance table: wide format showing DiD direction per cell type per gene
upset_combined_df %>%
...
save(combined_wide, file = file.path(results_dir, "model comparison", "celltype_deg_concordance.RData"))


# genes significant in 4+ cell types
gene_list_4 <- combined_wide %>%
  filter(n_total >= 4) %>%
  pull(Gene)

save(gene_list_4, file = file.path(results_dir, "model comparison", "gene_list_greater_than_3.RData"))
...
```{r echo = F}
# read IPA pathway analysis results for each cell type
for (cell in names(celltype_groups)){
  ipa_res <- read_excel(file.path(results_dir, "IPA/kpmp/nebula", paste0("full ", tolower(cell), "_kpmp_DiD.xls")), skip = 1)
...
```{r echo = F}
# horizontal bar plots of top 20 IPA pathways per cell type, colored by z-score
for (cell in names(celltype_groups)) {
  ...
  ggsave(file.path(results_dir, "Figures/Pathways/nebula/kpmp", paste0("full_", tolower(cell), "_ipa.png")))
}
```
# GSEA

```{r echo = F}
# run fGSEA on full gene set (hallmark, reactome, GO) and save results
for (cell in names(celltype_groups)) {
  df <- get(paste0(tolower(cell), "_kpmp"))
  ...
  saveRDS(gsea_list, file.path(results_dir, "GSEA", paste0("full_", tolower(cell), "_gsea.RDS")))
  
}

# export hallmark GSEA results as CSVs
for (cell in names(celltype_groups)) {
  rdsfile <- readRDS(file.path(results_dir, "GSEA", paste0("full_", tolower(cell), "_gsea.RDS")))
  hallmark_file <- rdsfile$hallmark %>%
    dplyr::select(-leadingEdge)
  write.csv(hallmark_file, file.path(results_dir, "GSEA", paste0("hallmark_", tolower(cell), "_gsea.csv")), row.names = F, na = "")
}
